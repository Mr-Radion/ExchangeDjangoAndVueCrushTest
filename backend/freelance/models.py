...
class Executor(models.Model):
    # каскадное удаление - это если удалится юзер, то удалятся все записи с его участием
    # в данном случае в этой модели
...
class Tag(models.Model):
    # ключ опциональный, тоесть null - допустимое значение, и blank - может быть пустым, необязат к заполнению
...
class Ordering(models.Model):
    # связываем клиента и исполнителя
...
class Message(models.Model):
    ...
    is_edited = models.BooleanField(default=False)  # дата редактирования сообщения
    desk = models.CharField(max_length=1000)


# для общения с тех поддержкой сайта
